<template>
  <div class="space-y-3">
    ...
    <v-data-table-server
      ...
      :items="leaveType"
      :items-length="leaveTypeStore.totalRecords"
      :items-per-page="leaveTypeStore.params.length"
      :loading="leaveTypeStore.isLoading"
      class="elevation-1 custom-header-table"
      @update:options="onTableOptionsChange"
    >
      <template #[`item.no`]="{ index }">
        {{ (leaveTypeStore.params.start ?? 0) + index + 1 }}
      </template>
      ...
      <template #[`item.actions`]="{ item }">
        <div class="flex justify-end items-center gap-1">
          ...
          <v-btn
            ...
            :loading="leaveTypeStore.isLoadingDestroy"
...
const leaveType = computed(() => leaveTypeStore.leaveType);
...
function onSearch(value: string | null) {
  ...
  searchTimeout = setTimeout(() => {
    leaveTypeStore.params.search = value ?? "";
    leaveTypeStore.params.start = 0;
    leaveTypeStore.fetchLeaveType();
  }, 400);
}

function onClear() {
  searchQuery.value = "";
  leaveTypeStore.params.search = "";
  leaveTypeStore.params.start = 0;
  leaveTypeStore.fetchLeaveType();
}
...
const headers = [
  ...
  { title: "Jenis Izin", key: "type", sortable: false },
  { title: "Full Day", key: "is_full_day", sortable: false },
...
function onTableOptionsChange(options: { page: number; itemsPerPage: number }) {
  leaveTypeStore.params.length = options.itemsPerPage;
  leaveTypeStore.params.start = (options.page - 1) * options.itemsPerPage;
  leaveTypeStore.fetchLeaveType();
}
...
async function deleteWorkExperience(id: number) {
  try {
    await leaveTypeStore.destroyLeaveType(id);
...
onMounted(() => leaveTypeStore.fetchLeaveType());
</script>
<style scoped>
